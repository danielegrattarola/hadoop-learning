...
parser.add_argument('--accept', type=float, default=999999., help='the run will be automatically discarded if the mean relative error is higher than the given threshold (Default: inf)')
...
# PANDAS GROUPBY TO COMPUTE RMSE BASED ON CORES AND DATASIZE (NEED TO CREATE A SINGLE CSV IN WHICH APPEND ALL PREDICTIONS)

prediction_data.to_csv(path_or_buf=logger.path + 'cross_validation_data_%s.csv' % args.dataset, mode='a')

# Remove everything if run was not good
if args.accept < os_MRE or str(raw_input('Keep run?')) is 'n':
